...
RUN env DEBIAN_FRONTEND=noninteractive apt-get update --no-install-recommends && \
    ...
    apt-get install -y --no-install-recommends python3-pip python3.9-dev python3-wheel && \
    pip3 install --no-cache-dir --upgrade pip && \
    pip3 install --no-cache-dir requests argparse cryptography==3.3.2 urllib3 && \
    pip3 install --no-cache-dir --upgrade requests && \
    pip3 install --no-cache-dir setuptools==58.4.0 && \
# Gramine
    apt-get update --fix-missing && \
    env DEBIAN_FRONTEND=noninteractive TZ=Etc/UTC apt-get install -y --no-install-recommends apt-utils wget unzip protobuf-compiler libgmp3-dev libmpfr-dev libmpfr-doc libmpc-dev && \
...
WORKDIR /gramine
RUN git checkout ${GRAMINE_BRANCH} && \
...
# meson will copy the original file instead of the symlink, which enable us to delete the gramine directory entirely
    ...
    apt-get install -y --no-install-recommends apt-utils vim curl nano wget unzip git tree zip && \
    apt-get install -y --no-install-recommends libsm6 make build-essential && \
    apt-get install -y --no-install-recommends autoconf gawk bison libcurl4-openssl-dev python3-protobuf libprotobuf-c-dev protobuf-c-compiler && \
    apt-get install -y --no-install-recommends netcat net-tools shellcheck && \
...
# Install sgxsdk and dcap, which is used for remote attestation
    mkdir -p /opt/intel/
WORKDIR /opt/intel
RUN wget -q https://download.01.org/intel-sgx/sgx-dcap/1.16/linux/distro/ubuntu20.04-server/sgx_linux_x64_sdk_2.19.100.3.bin && \
    chmod a+x ./sgx_linux_x64_sdk_2.19.100.3.bin && \
    printf "no\n/opt/intel\n"|./sgx_linux_x64_sdk_2.19.100.3.bin && \
    shellcheck /opt/intel/sgxsdk/environment && \
    . /opt/intel/sgxsdk/environment
WORKDIR /opt/intel
RUN wget -q https://download.01.org/intel-sgx/sgx-dcap/1.16/linux/distro/ubuntu20.04-server/sgx_debian_local_repo.tgz && \
...
WORKDIR /ppml
RUN bash /ppml/download_jars.sh
